...
def start_backup(path_postgres, user, password, path_backup, db, server, port, reindexdb,
    ...
    command = path_postgres + '/pg_dump' + ' ' + '--dbname=postgresql://' + user + ':' + password + '@' + server + ':' + port + '/' + db + ' ' + '> ' + path_backup + '/' + file_name + ' -v -F c 2> ' + path_backup + '/' + log_name  # формируем команду для бекапа
    os.system(command)
    ...
    if reindexdb == '1':
        ...
        # формируем команду
        command_reindexdb = path_postgres + '/reindexdb' + ' ' + '--dbname=postgresql://' + user + ':' + password + '@' + server + ':' \
    ...
    if vacuum == '1':
        ...
        # исполняем команду
        command_vacuum = path_postgres + '/vacuumdb' + ' ' + '--dbname=postgresql://' + user + ':' + password + '@' + server + ':' \
                         + port + '/' + db + ' ' + ' -v 2> ' + path_backup + '/' + log_name  # формируем команду для бекапа
        # выполняем команду
